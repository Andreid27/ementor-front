...
const QuizReviewAttempt = props => {
  ...
  const renderClient = user => {
    ...
    if (user.avatar && user.avatar.length) {
      return <CustomAvatar src={`/images/avatars/${user.avatar}`} sx={{ mr: 3, width: '2.5rem', height: '2.5rem' }} />
    } else {
      return (
  ...
  useEffect(() => {
    ...
    apiClient
      ...
      .then(response => {
        ...
        setUser(students.find(student => student.id === response.data.studentId))
            ...
            <CardContent>
              <Grid container spacing={6}>
                ...
                <Grid item xs={12} sm={3}>
                  <Box sx={{ marginTop: '-10rem', marginBottom: '1rem', marginLeft: '1rem' }}>
                    <CountdownTimer
