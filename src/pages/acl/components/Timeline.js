...
const CardActivityTimeline = (props) => {
  ...
  useEffect(() => {
    if (selectedStudentId) {
      setIsDrawerOpen(true) // Open drawer only when selectedStudentId is set
    }
  }, [selectedStudentId])

  const handleDrawerClose = () => {
    setIsDrawerOpen(false)
    setSelectedStudentId(null) // Reset selectedStudentId when drawer is closed
  }
                  ...
                  <TimelineContent sx={{ pt: 0, mt: 0, mb: (theme) => `${theme.spacing(2)} !important` }}>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                      <Avatar
                        ...
                        onClick={() => handleAvatarClick(quizStudent.studentId)}
                      />
                      <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                        ...
                        <Typography variant='caption'>
                          {quizStudent.endedTime ? (
                            ...
                            `Testul este în desfășurare, au mai rămas ${timeBetween(
                              quizStudent.startedAt,
                              quizStudent.endTime
                            )} `
                          )}
                        </Typography>
