...
import schoolSpecialities from '../static/schoolSpecialities.json'
import schoolDomains from '../static/schoolDomains.json'
...
const UserViewLeft = ({ profileData, profilePictureUrl, quizStats }) => {
  // ** States
  ...
  function getOverallResultsColor(data) {
    if (data && data.questions && data.questions.totalQuestions === 0) {
      return [0, 'info']
    }
    if (totalAvarage < 0) {
      return [Math.abs(totalAvarage), 'error']
    } else if (totalAvarage == 0) {
      return [totalAvarage, 'info']
    } else {
      if (totalAvarage < 50) {
        return [totalAvarage, 'error']
      }
      if (totalAvarage < 80) {
        return [totalAvarage, 'warning']
      }
  ...
  function getAdress(profileData) {
    if (profileData && profileData.address) {
      let address = ''
      if (profileData.address.countyValue) {
        address += profileData.address.countyValue + ', '
      }
      if (profileData.address.city) {
        address += profileData.address.city + ', '
      }
      if (profileData.address.state) {
        address += profileData.address.state + ', '
      }
      if (profileData.address.street) {
        address += profileData.address.street + ', '
      }
      if (profileData.address.number) {
        address += profileData.address.number
      }
      if (profileData.address.staircase) {
        address += ', ' + profileData.address.staircase
      }
      if (profileData.address.block) {
        address += ', ' + profileData.address.block
      }
      if (profileData.address.apartment) {
        address += ', ' + profileData.address.apartment
      }
            ...
            <CardContent sx={{ pt: theme => `${theme.spacing(2)} !important` }}>
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Box sx={{ mr: 8, display: 'flex', alignItems: 'center' }}>
                  ...
                  <div>
                    <Typography sx={{ fontWeight: 500, color: 'text.secondary' }}>{totalAvarage}</Typography>
            ...
            <CardContent sx={{ pb: 4 }}>
              ...
              <Box sx={{ pt: 4 }}>
                <Box sx={{ display: 'flex', mb: 3 }}>
                  ...
                  <Typography sx={{ color: 'text.secondary' }}>{profileData.user.email}</Typography>
                </Box>
                <Box sx={{ display: 'flex', mb: 3, alignItems: 'center' }}>
                  ...
                  <CustomChip
                    ...
                    skin='light'
                    size='small'
                    label={profileData.user.hasProfile ? 'active' : 'inactive'}
                    color={statusColors[profileData.user.hasProfile ? 'active' : 'inactive']}
                    sx={{
                ...
                <Box sx={{ display: 'flex', mb: 3 }}>
                  ...
                  <Typography sx={{ color: 'text.secondary' }}>{profileData.phone}</Typography>
                </Box>
                <Box sx={{ display: 'flex', mb: 3 }}>
                  <Typography sx={{ mr: 2, fontWeight: 500, color: 'text.secondary' }}>Adresă:</Typography>
                  <Typography sx={{ color: 'text.secondary' }}>{getAdress(profileData)}</Typography>
                </Box>
                <Box sx={{ display: 'flex', mb: 3 }}>
